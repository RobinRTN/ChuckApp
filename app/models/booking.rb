class Booking < ApplicationRecord
  ...
  scope :today, -> { where("start_time >= ? AND start_time < ? AND status = 'Accepted'", Time.now, Date.today.end_of_day).order(:start_time) }
  ...
  scope :passed, -> { where("start_time < ?", Time.now).order(:start_time) }
  scope :passed_confirmed, -> { where("start_time < ? AND status = 'Accepted'", Time.now).order(:start_time) }
  scope :upcoming, -> { where("start_time >= ?", Time.now).order(:start_time) }

  # scope to print out incoming pending bookings
  scope :today_pending, -> { where("start_time >= ? AND start_time < ? AND status = 'Pending'", Time.now, Date.today.end_of_day).order(:start_time) }
  ...
  scope :after_pending, -> { where("start_time >= ? AND status = 'Pending'", Time.now).order(:start_time) }
